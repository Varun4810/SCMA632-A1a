...
assmnew$Meals_At_Home <- impute_with_mean(assmnew$Meals_At_Home)
assmnew$Meals_Employer <- impute_with_mean(assmnew$Meals_Employer)
assmnew$Meals_Payment <- impute_with_mean(assmnew$Meals_Payment)
...
for (col in outlier_columns) {
  assmnew <- remove_outliers(assmnew, col)
}

# Summarize consumption
assmnew$total_consumption <- rowSums(assmnew[, c("ricepds_v", "Wheatpds_q", "chicken_q", "pulsep_q", "wheatos_q")], na.rm = TRUE)
...
district_mapping <- c("5" = "Barpeta", "6" = "Kamrup", "10" = "Nagaon")
sector_mapping <- c("2" = "URBAN", "1" = "RURAL")

assmnew$District <- as.character(assmnew$District)
assmnew$Sector <- as.character(assmnew$Sector)
assmnew$District <- ifelse(assmnew$District %in% names(district_mapping), district_mapping[assmnew$District], assmnew$District)
assmnew$Sector <- ifelse(assmnew$Sector %in% names(sector_mapping), sector_mapping[assmnew$Sector], assmnew$Sector)
